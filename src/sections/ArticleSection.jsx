    ...
    return (
        <section className="bg-transparent py-16">
            ...
            <div className="flex flex-col justify-center items-center text-center mb-8">
                <h2 className="text-white text-3xl poppins-bold">Artikel Terbaru</h2>
            ...
            <div className="flex justify-center space-x-8 px-8">
                {articles.map((article, index) => (
                    <Tilt
                        key={index}
                        className={`Tilt transition-all duration-300 ease-in-out ${activeArticle === index ? 'w-80 h-80' : 'w-64 h-64'}`}
                        tiltMaxAngleX={25}
                        tiltMaxAngleY={25}
                        scale={1.05}
                        transitionSpeed={300}
                    ...
                    >
                        <div className="Tilt-inner bg-white bg-opacity-10 backdrop-filter backdrop-blur-md shadow-lg rounded-xl p-4 border border-white/30 hover:bg-opacity-20 transition-all duration-300">
                            ...
                            <div className="overflow-hidden">
                                <img src={article.image} alt={article.title} className="w-full h-32 rounded-lg mb-4 object-cover" />
                            </div>
                            {/* 3D Floating Text */}
